...
REM   3. Esegue demo\old_test\seed_users.py per creare (o aggiornare, se gia'
REM      esistono) gli account demo di autenticazione — rieseguibile senza
REM      effetti collaterali: serve perche' demo\old_test\seed_test_scenario.py,
REM      dal passo successivo, fa login come l'account amministratore di seed
REM      prima di popolare la demo, e su un database nuovo quell'account
REM      non esisterebbe ancora.
REM   4. Esegue demo\old_test\seed_test_scenario.py (POST dirette via API,
REM      nessun Edge C++ richiesto) per popolare zone, ricette e piante in
...
echo [avvia_demo] Creo/aggiorno gli account demo con demo\old_test\seed_users.py...
"%PYTHON%" "%ROOT%demo\old_test\seed_users.py"
if %ERRORLEVEL% NEQ 0 (
...
echo [avvia_demo] Popolo la demo con demo\old_test\seed_test_scenario.py (test RAPIDO, nessun Edge)...
"%PYTHON%" "%ROOT%demo\old_test\seed_test_scenario.py"
if %ERRORLEVEL% NEQ 0 (
